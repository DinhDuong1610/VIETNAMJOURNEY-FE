    ...
    if (group_id && user_id) {
      fetch(`${API_BASE_URL}api/checkMemberMeeting`, {
        ...
        .then((data) => {
          if (data.result === "yes") {
            setUserInfo([
              { userId: user_id, name: data.name, image: data.image },
            ]);
              ...
              if (
                ...
                setUserInfo((prevInfo) => {
                  ...
                  message.users.forEach((newUser) => {
                    const userExists = prevInfo.some(
                  ...
                  const usersToExit = prevInfo.filter(
                    (user) => !newUserIds.includes(user.userId)
                  );
                  usersToExit.forEach((user) => {
                    setExitingUsers((prevExiting) => [
                      ...prevExiting,
                      user.userId,
                    ]);
                    setTimeout(() => {
                      ...
                      setExitingUsers((prevExiting) =>
                        prevExiting.filter((id) => id !== user.userId)
                      );
                    }, 500);
                  });
          ...
          {userInfo.map((user, index) => (
            <div
              ...
              className={`${styles.userInfo} ${
                exitingUsers.includes(user.userId) ? styles.exit : ""
              }`}
            >
              <img
                alt="avatar"
              ...
              <div className={styles.details}>
                <p style={{ fontWeight: 600 }}>
                  {user.name}{" "}
                  {user.userId === adminInfo.userId.toString() && (
                    <i
                ...
                <span style={{ lineHeight: "0px", fontWeight: 500 }}>
                  {user.userId === adminInfo.userId.toString()
                    ? "Quản trị viên"
                    : "Thành viên"}
                </span>
