    ...
    try {
      ...
      if (response.ok) {
        ...
        onMeetingCreated(`Nhóm đang có 1 cuộc họp diễn ra ngay bây giờ.
                Link tham gia : http://localhost:3000/#/VideoChat?group_id=${group_id}&thread=${data.id}`);
      } else {
...
function ScheduleMeeting({ onMeetingCreated }) {
  // Nhận callback
      ...
      {!scheduleSuccessful ? (
        ...
        <div className={styles.meetingInfo}>
          ...
          <p>
            Link cuộc họp:{" "}
            <a
              href={`http://localhost:3000/#/VideoChat?group_id=${group_id}&thread=${meetingData.id}`}
            >{`http://localhost:3000/#/VideoChat?group_id=${group_id}&thread=${meetingData.id}`}</a>
          </p>
          <p>
            <strong>Thời gian:</strong>{" "}
            {new Date(meetingData.date).toLocaleString()}
          </p>
        </div>
      )}
    </div>
...
function Meeting({ onMeetingCreated }) {
  // Nhận callback
      ...
      {!selectedOption ? (
        <>
          ...
          <button className={styles.meetingButton} onClick={handleMeetingNow}>
            <i class="fa-solid fa-plus fa-beat-fade"></i> Bắt đầu cuộc họp tức
            thì
          </button>
          <button
